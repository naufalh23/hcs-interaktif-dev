...
const STATUS_PILL: Record<string, string> = {
  available: "bg-[rgba(20,160,80,0.82)] text-white",
  indent:    "bg-[rgba(201,120,10,0.82)] text-white",
  sold:      "bg-[rgba(176,48,48,0.82)] text-white",
};
...
export default function UnitPreviewCard({
  unit: u,
  onOpenLightbox,
}: {
  unit: Unit;
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-[0_4px_16px_rgba(27,94,53,0.12)]">

      {/* ── Carousel ──────────────────────────────────── */}
      <div className="relative h-44 bg-[#163F25] overflow-hidden group">
        <div className="absolute inset-0 bg-linear-to-br from-[#1B5E35] via-[#2E7D52] to-[#163F25]" />

        {imgs.length > 0 && (
          <div
            className="absolute inset-0 cursor-zoom-in"
            onClick={() => imgs[imgIdx] && onOpenLightbox(imgs[imgIdx])}
          >
            <Image
              key={imgs[imgIdx]}
              src={imgs[imgIdx]}
        ...
        <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 px-2.5 py-1 rounded-full
                        bg-white/15 backdrop-blur-[3px] pointer-events-none">
          <div className="w-4 h-4 rounded-full bg-white flex items-center justify-center text-[#1B5E35] text-[9px] font-bold">
            ✓
          </div>
          <span className="text-[9px] font-semibold text-white tracking-[0.5px]">Unit Dipilih</span>
        </div>
      ...
      {/* ── Info panel ────────────────────────────────── */}
      <div className="bg-[#FDFAF4] px-5 pt-3.5 pb-4">
        <div className="flex justify-between items-start mb-2.5">
          <div className="font-serif font-bold text-[22px] text-[#163F25] leading-none">{u.code}</div>
          <div className="text-right">
            <div className="text-[8px] text-[#9AAD9C] tracking-[0.5px] uppercase mb-0.5">Harga</div>
            <div className="font-bold text-[14px] text-[#163F25] leading-none">{formatPrice(u.price)}</div>
          </div>
        </div>
