...
export default function IdleOverlay({ idleMs }: { idleMs: number }) {
  ...
  const cd   = useRef<ReturnType<typeof setInterval>>()

  const clearAll = () => { clearTimeout(idle.current); clearInterval(cd.current) }

  const start = () => {
    clearAll()
    if (screen === 'attract' || screen === 'success') return
    idle.current = setTimeout(() => {
      let n = CD_SECS; setCount(n); setShow(true)
      cd.current = setInterval(() => {
        n--; setCount(n)
        if (n <= 0) { clearAll(); setShow(false); reset() }
      }, 1000)
    }, idleMs)
  }

  useEffect(() => { start(); return clearAll }, [screen]) // eslint-disable-line
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-4"
      ...
      <p style={{ fontSize: '10px', letterSpacing: '4px', textTransform: 'uppercase', fontWeight: 600, color: '#9AAD9C', marginBottom: 4 }}>
        Sesi akan berakhir dalam
      </p>

      <div className="font-serif font-bold leading-none animate-fade-up"
        style={{ fontSize: '100px', color: '#1B5E35', lineHeight: 1 }}>
        {count}
      </div>

      <button
        onClick={() => { clearAll(); setShow(false); start() }}
        ...
        style={{
          ...
          color: '#F5F0E8', fontSize: '11.5px', fontWeight: 700,
